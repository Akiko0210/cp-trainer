...
-- ============================================================================
-- The inbox (db/migrations/007_inbox.sql).
--
-- One row per (recipient, duel, kind), written by a trigger on duels so every
-- transition — the app's, the worker's, arena_sweep()'s — produces it. The
-- header bell, the top-right popup and the shown-on-login challenge all read
-- from here; the live stream carries `recipient_id` so only that person's
-- tab is told. Round wins are not rows: they are transient, in the room.
-- ============================================================================
