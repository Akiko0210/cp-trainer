    ...
    if (review.length > 0) {
      ...
      why =
        review[0].reason === "stale"
          ? // Elapsed time, not a calendar date: this string is built on the
            // server, where a formatted date carries the server's zone and
            // reads a day off for anyone east of it. How long ago is the same
            // number everywhere.
            `${review[0].name} is going stale — last practiced ${daysAgo(review[0].last_practiced_at)}.`
